...
dfTargetVariables <- subset(dfCPSComputerSupplement, select=c("CyberBullied", "Age", "FamilyIncome", "USState", "Race", "Gender", "MetroAreaSizeCode", "EducationLevelCode", "HasDisability", "HaveHighSpeedInternet","HaveLaptop","HaveSmartPhone", "HighestConcernisBullying"))
...
The descriptive statistics and conditional means calculated above leads to a regression predicting whether or not a respondent will encounter cyberbullying should include the following predictors and covariates: family income levels, age, state where respondent lives, and Educational Level. Other variables were dropped due to little suspected correlation given the above visualizations mean calculations. Model test data was further limited to 2019 census information due to size of data set.
...
dfTargetVariables <- subset(dfTargetVariables, select=c("CyberBullied", "Age", "FamilyIncome", "USState", "EducationLevelCode"))
dfTargetVariables <- dfTargetVariables %>% mutate(across(where(is.character), as.integer))
