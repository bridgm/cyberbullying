...
```{r CyberbullyByIncome, include=FALSE}
...
States_Bullied <- dfCPSComputerSupplement %>%  count(StateAbbreviation, CyberBulliedAnswer, InterviewYear)
States_Bullied <-
  filter(States_Bullied,
         CyberBulliedAnswer == "Yes" & InterviewYear == "2019")
...
States_Bullied.heatmap <- States_Bullied %>%
  ...
  geom_sf() +
  ggtitle(label = "Cyberbullying Experienced in the US by State") +
  theme(
    axis.text.x = element_blank(),
    axis.ticks.x = element_blank(),
    plot.title = element_text(hjust = 0.5),
